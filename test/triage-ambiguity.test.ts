...
import { ambiguityStage } from '../src/triage/stages/ambiguity.js'
...
test('clear, targeted task scores near 1 with no flags', () => {
  const { patch } = ambiguityStage.execute(ctx('fix the null check in src/auth.ts and update the tests'))
...
test('edit verb with no target flags missing-target', () => {
  const { patch } = ambiguityStage.execute(ctx('please refactor everything to be better'))
...
test('very short task flags underspecified-goal', () => {
  const { patch } = ambiguityStage.execute(ctx('fix it'))
  assert.ok(patch.ambiguity!.flags.includes('underspecified-goal'))
})

test('vague quantifier is flagged', () => {
  const { patch } = ambiguityStage.execute(ctx('update the config and some other stuff in server.ts'))
  assert.ok(patch.ambiguity!.flags.includes('vague-quantifier'))
})

test('score is clamped to [0,1] and deterministic', () => {
  const { patch } = ambiguityStage.execute(ctx('fix it but somehow add and remove stuff'))
  assert.ok(patch.ambiguity!.score >= 0 && patch.ambiguity!.score <= 1)
  const again = ambiguityStage.execute(ctx('fix it but somehow add and remove stuff'))
